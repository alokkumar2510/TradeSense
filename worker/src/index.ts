...
import { handleQuote, handleHistory, handleSignal, handleSearch } from "./routes/stock";
...
const worker = {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // ─── Routes requiring symbol: /api/quote/RELIANCE.NS ─────────────────────
    ...
    if (symbolMatch && method === "GET") {
      ...
      switch (route) {
        ...
        case "history": return handleHistory(symbol, env);
